...
- **Administración de usuarios (`usuarios.html`)**
  ...
  * La contraseña se guarda cifrada con BCrypt; el correo debe ser único (no se permiten altas duplicadas, ni con mayúsculas/minúsculas distintas).
...
│     ├──   UsuarioController.java                 #  API de administración de usuarios (/api/usuarios, solo ADMINISTRADOR)
...
│     └──   *Repository.java                        #  Repositorios Spring Data (incluye RolRepository, UsuarioRepository)
│
├──   src/main/resources/
│     │
│     ├──   application.properties                  #  Conexión a la BD vía variables de entorno
│     └──   static/                                  #  login.html, index.html, nueva-solicitud.html, usuarios.html,
│                                                     #  app.js, nueva-solicitud.js, usuarios.js, style.css
│
...
- **Usuarios:** un `ADMINISTRADOR` ya puede crear `FUNCIONARIO` y `TECNICO` desde `usuarios.html`, pero todavía no hay forma de editarlos, desactivarlos ni restablecer su contraseña desde la interfaz (ver el SQL del paso 4 para eso).
